...
def analyze(request):
    # get the text
    djtext=request.GET.get('value','default')
    sh=request.GET.get('vsearch','default')
    li = djtext.split(" ")
    print(li)
    ...
    out1, state1 = tree.BFS(int(sh))
    out2, state2 = tree.DFS(int(sh))
    params = { 'tree': out1, 'ans' : state1, 'ans2': state2,'s_ele':sh}
